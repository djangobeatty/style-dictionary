  ...
  describe('loadFile', () => {
    ...
    it('should throw error if it tries to import TS files with unsupported Node env', async () => {
      if (isNode) {
        ...
        let tokens;
        try {
          tokens = await loadFile('__tests__/__json_files/tokens.ts');
        } catch (e) {
          err = e;
        }
        if (err) {
          ...
          // The runtime imports TypeScript natively (Bun, Deno, or Node.js with type
          // stripping enabled), so the default export should have been loaded instead
          expect(tokens).to.deep.equal({
            colors: { $type: 'color', red: { 500: { $value: '#ff0000' } } },
          });
        }
      }
    });
