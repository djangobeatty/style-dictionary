...
1. **Never record a warning for a reference you are not emitting.** Filtered-out-reference warnings are collected globally during the format step and cleared only by the token that actually emits the reference. Any lookup that merely inspects tokens — reference-safe ordering, or an `outputReferences` predicate deciding whether to emit — must pass `recordWarnings: false` to `getReferences`; a token that is never formatted never clears the warning it recorded, so the leftover warning is reported as a false positive for the whole file.
