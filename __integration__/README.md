...
The integration tests still use Mocha + Chai and snapshots, but rather than singling out format code, each test group builds a complete Style Dictionary and then tests the content of the output files with snapshots and validating syntax where possible.

Run them with `npm run test:node`, or a single file with `npx mocha -r mocha-hooks.mjs <file>`. The `mocha-hooks.mjs` root hook loads the Chai plugins (`chai-as-promised` and the snapshot plugin), so omitting `-r mocha-hooks.mjs` leaves the assertions undefined.
