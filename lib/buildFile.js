...
import { isVerbose, shouldLog, verboseHint } from './utils/logging.js';
...
export default async function buildFile(file, platform = {}, dictionary, options) {
  ...
  let { format } = file || {};
  const verbosity = platform?.verbosity ?? options?.verbosity;
  ...
  if ((nested || tokenNamesCollisionCount === 0) && filteredReferencesCount === 0) {
    ...
    if (tokenNamesCollisionCount > 0) {
      ...
      const title = `While building ${chalk.rgb(255, 69, 0).bold(destination)}, ${
        tokenNamesCollisionCount === 1
          ? '1 token name collision was found'
          : `${tokenNamesCollisionCount} token name collisions were found`
      ...
      // by default only a concise message is shown, verbose logging
      // shows every individual collision
      const warn = isVerbose(verbosity)
        ? `${warnHeader}\n${title}\n    ${tokenNamesCollisionWarnings.join('\n    ')}\n${help}`
        : `${warnHeader}\n${title} ${verboseHint()}`;
      if (platform?.log === 'error') {
        throw new Error(warn);
      } else if (shouldLog(verbosity)) {
        // eslint-disable-next-line no-console
    ...
    if (filteredReferencesCount > 0) {
      ...
      const title = `While building ${chalk.rgb(255, 69, 0).bold(destination)}, ${
        filteredReferencesCount === 1
          ? '1 filtered out token reference was found'
          : `${filteredReferencesCount} filtered out token references were found`
      }; output may be unexpected.`;
      const help = chalk.rgb(
        255,
        165,
      ...
      // by default only a concise message is shown, verbose logging
      // shows every individual filtered out reference
      const warn = isVerbose(verbosity)
        ? `${warnHeader}\n${title} Here are the references that are used but not defined in the file:\n    ${filteredReferencesWarnings.join(
            '\n    ',
          )}\n${help}`
        : `${warnHeader}\n${title} ${verboseHint()}`;
      if (platform?.log === 'error') {
