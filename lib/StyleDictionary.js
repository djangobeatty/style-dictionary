...
import { defaultVerbosity, formatMessages, shouldLog } from './utils/logging.js';
...
export default class StyleDictionary extends Register {
  ...
  /**
   * @param {{ init?: boolean, verbosity?: Verbosity }} [opts] verbosity passed here (e.g. through the CLI) takes precedence over the `verbosity` config option
   */
  constructor(config = {}, opts = {}) {
    super();
    const { init = true, verbosity } = opts;
    /** @type {'warn'|'error'} */
    this.log = 'warn';
    /** @type {Verbosity} */
    this.verbosity = defaultVerbosity;
    /** @type {Verbosity|undefined} */
    this.verbosityOverride = verbosity;
    this.config = config;
    this.options = {};
  ...
  async extend(config = this.config, mutateOriginal = false) {
    ...
    Object.entries(options).forEach(([key, val]) => {
      ...
      /** @type {any} */ (this[_key]) = val;
    });
    // verbosity passed programmatically (e.g. --verbose or --silent in the CLI)
    // takes precedence over the verbosity set in the config
    this.verbosity = this.verbosityOverride ?? this.verbosity ?? defaultVerbosity;
    this.options = { ...options, verbosity: this.verbosity };
    ...
    if (this.options.source) {
      ...
      const result = await combineJSON(
        ...
        true,
        /**
         * Only colliding token values are reported: the side data we add to
         * tokens (filePath, isSource) as well as metadata like type and comment
         * colliding is not interesting to the user and only bloats the logs.
         * @param {Token} prop
         */
        function Collision(prop) {
          const valueKeys = ['value', '$value'];
          const inValue = valueKeys.includes(prop.path[prop.path.length - 1]);
          if (!valueKeys.includes(prop.key) && !inValue) {
            return;
          }
          // tokens know which file they came from, which helps tracing
          // where a collision originates from
          const originalFile = prop.target.filePath ? ` (${prop.target.filePath})` : '';
          const newFile = prop.copy.filePath ? ` (${prop.copy.filePath})` : '';
          GroupMessages.add(
            PROPERTY_VALUE_COLLISIONS,
            `Collision detected at: ${prop.path.join('.')}! Original value: ${
              prop.target[prop.key]
            }${originalFile}, New value: ${prop.copy[prop.key]}${newFile}`,
          );
        },
        true,
        this.parsers,
      ...
      if (GroupMessages.count(PROPERTY_VALUE_COLLISIONS) > 0) {
        ...
        const warn = `\n${PROPERTY_VALUE_COLLISIONS}:\n${formatMessages(
          collisions,
          'property value collision',
          this.verbosity,
        )}\n\n`;
        if (options.log === 'error') {
          throw new Error(warn);
        } else if (shouldLog(this.verbosity)) {
  ...
  async exportPlatform(platform) {
    ...
    if (GroupMessages.count(PROPERTY_REFERENCE_WARNINGS) > 0) {
      const refErrors = GroupMessages.flush(PROPERTY_REFERENCE_WARNINGS);
      throw new Error(
