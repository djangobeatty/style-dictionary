...
import { isVerbose, log, logError, logWarning } from './utils/log.js';
...
const PROPERTY_VALUE_COLLISION_PATHS = GroupMessages.GROUP.PropertyValueCollisionPaths;
...
export default class StyleDictionary extends Register {
  ...
  constructor(config = {}, { init = true } = {}) {
    super();
    /** @type {import('../types/Config.d.ts').LogConfig} */
    this.log = {};
    this.config = config;
    this.options = {};
    ...
    if (this.options.source) {
      ...
      const result = await combineJSON(
        ...
        function Collision(prop) {
          GroupMessages.add(PROPERTY_VALUE_COLLISION_PATHS, prop.path.join('.'));
          GroupMessages.add(
      ...
      if (GroupMessages.count(PROPERTY_VALUE_COLLISIONS) > 0) {
        ...
        const collisionPaths = GroupMessages.flush(PROPERTY_VALUE_COLLISION_PATHS);
        let warn;
        if (isVerbose(options.log)) {
          warn = `\n${PROPERTY_VALUE_COLLISIONS}:\n${collisions.join('\n')}\n\n`;
        } else {
          warn = `\n${PROPERTY_VALUE_COLLISIONS}:\nCollisions detected at: ${collisionPaths.join(
            ', ',
          )}\n\nRun with --verbose to see the full details of these collisions.\n\n`;
        }
        logWarning(warn, options.log);
      }
    }
  ...
  async exportPlatform(platform) {
    ...
    if (GroupMessages.count(PROPERTY_REFERENCE_WARNINGS) > 0) {
      ...
      // respect platform-specific log config, fall back to the global config
      const logConfig = platformConfig.log ?? this.options.log;
      const msg = isVerbose(logConfig)
        ? `\n${PROPERTY_REFERENCE_WARNINGS}:\n${warnings.join(
            '\n',
          )}\n\nProblems were found when trying to resolve property references`
        : `\n${PROPERTY_REFERENCE_WARNINGS}: ${warnings.length} broken reference${
            warnings.length === 1 ? '' : 's'
          } found. Run with --verbose to see the details.\n\nProblems were found when trying to resolve property references`;
      logError(msg, logConfig);
    }
  ...
  async getPlatform(platform) {
    await this.hasInitialized;
    log('\n' + platform, this.options?.platforms?.[platform]?.log ?? this.options.log);
