...
import GroupMessages, {
  GroupMessages as GroupMessageCollector,
  verbosityInfo,
} from './utils/groupMessages.js';
...
export default class StyleDictionary extends Register {
  ...
  async formatFile(file, platform, dictionary) {
    ...
    if (platform.buildPath && fullDestination) {
      fullDestination = joinPath(platform.buildPath, fullDestination);
    } else if (fullDestination) {
      fullDestination = normalizeSeparators(fullDestination);
    }
    ...
    // The format of this file and its helpers record warnings about
    // references the filter excluded into this collector, which belongs to
    // this file alone. Platforms and their files are formatted concurrently,
    // so a shared collector would let a warning recorded by one file be
    // reported for whichever sibling finishes first. It is a non-enumerable
    // property so it stays out of the dictionary that formats serialize.
    const filteredReferences = new GroupMessageCollector();
    Object.defineProperty(filteredDictionary, 'filteredReferences', {
    ...
    const filteredReferencesCount = filteredReferences.count(FILTER_WARNINGS);
    ...
    if (
      ...
      if (filteredReferencesCount > 0) {
        const filteredReferencesWarnings = filteredReferences
          .fetchMessages(FILTER_WARNINGS)
          .join('\n    ');
        const title = `While building ${chalk
          .rgb(255, 69, 0)
          .bold(
