...
export default class StyleDictionary extends Register {
   ...
   * @param {object} [opts]
   * @param {boolean} [opts.init] - whether to immediately call the async extend function
   * @param {LogConfig['warnings']} [opts.warnings] - takes precedence over `config.log.warnings`
   * @param {LogConfig['verbosity']} [opts.verbosity] - takes precedence over `config.log.verbosity`
   */
  constructor(config = {}, { init = true, ...logOverrides } = {}) {
    super();
    /**
     * Log config passed to the constructor, which wins over the one in the config object.
     * @type {LogConfig}
     */
    this.logOverrides = asLogConfig(logOverrides);
    /** @type {ResolvedLogConfig} */
    this.log = mergeLogConfigs(this.logOverrides);
    this.config = config;
    this.options = {};
  ...
  async extend(config = this.config, mutateOriginal = false) {
    ...
    if (!mutateOriginal) {
      const newSD = new StyleDictionary(
        deepmerge(this.options, /** @type {Config} */ (config)),
        // constructor log options are overrides, so they have to be passed on
        { init: false, ...this.logOverrides },
      );
      return newSD.init();
    }
    ...
    // resolve the log config so that the rest of the codebase can rely on
    // every log property being set, CLI options win over the config object
    this.log = mergeLogConfigs(options.log, this.logOverrides);
    this.options = { ...options, log: this.log };
    ...
    if (this.options.source) {
      ...
      /**
       * A single token colliding usually collides on multiple of its properties
       * (value, type, filePath, ...), which used to result in a handful of nearly
       * identical messages per token. We only report the first collision of a
       * token, at which point the target token is still untouched, so we can
       * tell the user which definition won and which file it came from.
       * @type {Set<string>}
       */
      const collidedTokens = new Set();
      ...
      const result = await combineJSON(
        ...
        function Collision(prop) {
          ...
          if (collidedTokens.has(path)) {
            return;
          }
          collidedTokens.add(path);
          const valProp = Object.hasOwn(prop.target, '$value') ? '$value' : 'value';
          GroupMessages.add(
            PROPERTY_VALUE_COLLISIONS,
            [
              path,
              `  Was: ${prop.target[valProp]}${inFile(prop.target.filePath)}`,
              `  Now: ${prop.copy[valProp]}${inFile(prop.copy.filePath)}`,
            ].join('\n'),
          );
        },
        true,
      ...
      if (GroupMessages.count(PROPERTY_VALUE_COLLISIONS) > 0) {
        const collisions = GroupMessages.flush(PROPERTY_VALUE_COLLISIONS);
        logWarning(
          composeMessage(`Token collisions detected (${collisions.length}):`, collisions, this.log),
          this.log,
        );
      }
    }
  ...
  async exportPlatform(platform) {
    ...
    if (GroupMessages.count(PROPERTY_REFERENCE_WARNINGS) > 0) {
      const warnings = GroupMessages.flush(PROPERTY_REFERENCE_WARNINGS);
      // reference errors always throw, the verbosity only determines whether
      // every single one of them is listed or just how many there are
      throw new Error(
        composeMessage(
          `Reference Errors:\nSome token references (${warnings.length}) could not be found.`,
          warnings,
          mergeLogConfigs(platformConfig.log),
        ),
      );
    }
  ...
  async getPlatform(platform) {
    ...
    logInfo(
      '\n' + platform,
      mergeLogConfigs(this.log, this.options?.platforms?.[platform]?.log, this.logOverrides),
    );
