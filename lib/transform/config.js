...
import { mergeLogConfigs } from '../logging/index.js';
...
export default function transformConfig(platformConfig, dictionary, platformName) {
  ...
  // a platform only has to define the log properties it wants to deviate on,
  // the rest is inherited from the dictionary, CLI options always win
  const log = mergeLogConfigs(dictionary.log, platformConfig.log, dictionary.logOverrides);
  to_ret.log = log;
...
`;
  ...
  to_ret.files = (to_ret.files || []).map(function (file) {
    ...
    if (file.format) {
       ...
       * Only later will it be transformed to contain the formatter function
       */
      const format = /** @type {string}  */ (file.format);
      if (dictionary.format[format]) {
        ext.format = dictionary.format[format];
      } else {
  ...
  to_ret.actions = actions.map(
    /** @param {string} action */ function (action) {
      if (
        typeof dictionary.action[action].undo !== 'function' &&
