...
import { shouldLog } from '../utils/logging.js';
...
export default function transformConfig(platformConfig, dictionary, platformName) {
  ...
  to_ret.verbosity = platformConfig.verbosity ?? dictionary.verbosity;
...
`;
  ...
  to_ret.actions = actions.map(
    /** @param {string} action */ function (action) {
      if (typeof dictionary.action[action].undo !== 'function' && shouldLog(dictionary.verbosity)) {
