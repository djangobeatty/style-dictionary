 ...
 * @example
 * ```javascript
 * dictionary.allTokens.sort(sortByReference(dictionary.tokens, { usesDtcg: options.usesDtcg }))
 * ```
 * @param {Tokens} tokens
...
export default function sortByReference(tokens, opts) {
  ...
  /**
   * Grabs the original (pre-transform) value of a token, accounting for
   * DTCG syntax where the value lives under `$value` rather than `value`.
   * @param {Token} token
   * @returns {any}
   */
  ...
  function sorter(a, b) {
    ...
    // read on..
    if (a.original && usesReferences(originalValue(a))) {
      // Both a and b have references, we need to see if they reference each other
      if (b.original && usesReferences(originalValue(b))) {
