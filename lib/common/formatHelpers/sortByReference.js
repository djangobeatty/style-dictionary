...
export default function sortByReference(tokens, opts) {
  const { unfilteredTokens, usesDtcg } = opts ?? {};

  /**
   * Reads the original value of a token, accounting for DTCG syntax
   * where the original value is stored on `$value` rather than `value`.
   * @param {Token} token
   * @returns {any}
   */
