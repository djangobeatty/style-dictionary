...
export default function sortByReference(tokens, opts) {
  ...
  // DTCG tokens store their value under `$value` instead of `value`
  ...
  function sorter(a, b) {
    ...
    // read on..
    if (a.original && usesReferences(a.original[valueProp])) {
      // Both a and b have references, we need to see if they reference each other
      if (b.original && usesReferences(b.original[valueProp])) {
        const aRefs = getReferences(a.original[valueProp], tokens, {
          unfilteredTokens,
          warnImmediately: false,
          usesDtcg,
        });
        const bRefs = getReferences(b.original[valueProp], tokens, {
          unfilteredTokens,
          warnImmediately: false,
          usesDtcg,
        });
