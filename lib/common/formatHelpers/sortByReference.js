...
export default function sortByReference(tokens, opts) {
  ...
  // DTCG tokens store their value under `$value` rather than `value`
  const valueProp = usesDtcg ? '$value' : 'value';

  /**
   * Get the original (pre-transform) value of a token, taking the DTCG
   * `$value` property into account.
   * @param {Token} token
   * @returns {any}
   */
  function getOriginalValue(token) {
    return token.original ? token.original[valueProp] : undefined;
  }
  ...
  function sorter(a, b) {
    ...
    // read on..
    if (a.original && usesReferences(getOriginalValue(a))) {
      // Both a and b have references, we need to see if they reference each other
      if (b.original && usesReferences(getOriginalValue(b))) {
        const aRefs = getReferences(getOriginalValue(a), tokens, {
          unfilteredTokens: opts?.unfilteredTokens,
          usesDtcg,
          warnImmediately: false,
        });
        const bRefs = getReferences(getOriginalValue(b), tokens, {
          unfilteredTokens: opts?.unfilteredTokens,
          usesDtcg,
          warnImmediately: false,
        });
