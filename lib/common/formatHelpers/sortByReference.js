...
export default function sortByReference(tokens, opts) {
  const usesDtcg = opts?.usesDtcg ?? false;

  /**
   * Grab the original (pre-transform) value of a token, taking
   * DTCG syntax ($value) into account.
   * @param {Token} token
   * @returns {any}
   */
  function originalValue(token) {
    return usesDtcg ? token.original?.$value : token.original?.value;
  }
  ...
  function sorter(a, b) {
    const aComesFirst = -1;
    const bComesFirst = 1;
    // keep the tokens in their current order, Array.prototype.sort is stable
    const keepOrder = 0;
    ...
    const aUsesRefs = usesReferences(originalValue(a));
    const bUsesRefs = usesReferences(originalValue(b));

    // neither token has a reference, there is nothing to sort by
    ...
    const aRefs = getReferences(originalValue(a), tokens, {
      unfilteredTokens: opts?.unfilteredTokens,
      usesDtcg,
      warnImmediately: false,
    });
    const bRefs = getReferences(originalValue(b), tokens, {
      unfilteredTokens: opts?.unfilteredTokens,
      usesDtcg,
      warnImmediately: false,
    });
