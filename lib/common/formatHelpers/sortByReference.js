...
export default function sortByReference(tokens, opts) {
  ...
  function sorter(a, b) {
    ...
    // In DTCG syntax the value lives on `$value`, otherwise on `value`
    const aValue = a.original && (usesDtcg ? a.original.$value : a.original.value);
    const bValue = b.original && (usesDtcg ? b.original.$value : b.original.value);

    // If token a uses a reference and token b doesn't, b might come before a
    // read on..
    if (aValue && usesReferences(aValue)) {
      // Both a and b have references, we need to see if they reference each other
      if (bValue && usesReferences(bValue)) {
        const aRefs = getReferences(aValue, tokens, {
          unfilteredTokens: opts?.unfilteredTokens,
          warnImmediately: false,
          usesDtcg,
        });
        const bRefs = getReferences(bValue, tokens, {
          unfilteredTokens: opts?.unfilteredTokens,
          warnImmediately: false,
          usesDtcg,
        });
