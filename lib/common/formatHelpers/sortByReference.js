 ...
 * ```
 * @param {Tokens} tokens
 * @param {{unfilteredTokens?: Tokens, usesDtcg?: boolean, filteredReferences?: import('../../../types/DesignToken.d.ts').FilteredReferences}} [opts]
 * @returns {(a: Token, b: Token) => number}
 */
export default function sortByReference(
  tokens,
  { unfilteredTokens, usesDtcg, filteredReferences } = {},
) {
   ...
   * @param {Token} a
   * @param {Token} b
   * @param {{ recordWarnings?: boolean }} [opts]
   * @returns {number}
   */
  function sorter(a, b, { recordWarnings = true } = {}) {
    ...
    if (aUsesRefs && bUsesRefs) {
      ...
      // Only the tokens being sorted are ever formatted, so a reference
      // discovered while recursing must not be reported as a filtered-out
      // reference: the token holding it may itself be filtered out, in which
      // case nothing ever clears the warning again.
      return sorter(aRefs[0], bRefs[0], { recordWarnings: false });
    }
