...
/**
 * A single column of a markdown table output.
 * @typedef {Object} MarkdownTableColumn
 * @property {string} property - The token property to display, e.g. `name`, `value`, `type`, `comment`
 * @property {string} [header] - The column header label, defaults to the property name
 */

/**
 * The columns used by the `markdown/tables` format when none are configured.
 * @type {MarkdownTableColumn[]}
 */
const defaultMarkdownTableColumns = [
...
/**
 * Reads a token property, falling back to the DTCG `$`-prefixed variant so that
 * `value` and `type` work for both DTCG and legacy tokens.
 * @param {Token} token
 * @param {string} property
 * @returns {unknown}
 */
function getTokenProperty(token, property) {
...
/**
 * Renders a token property as a plain string. Composite values (objects and
 * arrays, e.g. shadow or typography tokens) are stringified so that they do not
 * end up as `[object Object]`.
 * @param {unknown} value
 * @returns {string}
 */
function stringifyMarkdownCell(value) {
...
/**
 * Escapes a value so it can safely live inside a markdown table cell:
 * line breaks are collapsed and pipes are escaped, so a single value can never
 * split a row into extra columns or terminate the table early.
 * @param {string} value
 * @returns {string}
 */
...
/**
 * Builds a single markdown table row from an array of already escaped cells.
 * @param {string[]} cells
 * @returns {string}
 */
...
/**
 * Normalizes the configured columns option into a list of column definitions.
 * @param {(MarkdownTableColumn | string)[]} [columns]
 * @returns {Required<MarkdownTableColumn>[]}
 */
function normalizeMarkdownTableColumns(columns) {
  if (!Array.isArray(columns) || columns.length === 0) {
    return /** @type {Required<MarkdownTableColumn>[]} */ (defaultMarkdownTableColumns);
  }
  return columns.map((column) => {
    if (typeof column === 'string') {
      return { property: column, header: column };
    }
    return { property: column.property, header: column.header ?? column.property };
  });
}
...
declare const ${moduleName}: ${JSON.stringify(treeWalker(dictionary.tokens), null, 2)}`;
   ...
   *
   * By default each row shows the token name, its resolved value and its type.
   * The columns can be configured per file through `options.columns`, where each
   * entry is either a token property name or an object with a `property` and a
   * `header` label. Pipes and line breaks in names or values are escaped so a
   * value can never split a row or terminate the table.
   *
   * @memberof Formats
   * @kind member
   * @typedef {Object} markdownTablesOpts
   * @property {(MarkdownTableColumn | string)[]} [markdownTablesOpts.columns] - The columns to render, in order. Defaults to name, value and type.
   * @param {FormatArgs & { options?: markdownTablesOpts }} options
   * @example
   * ```md
   * | Name | Value | Type |
   * | --- | --- | --- |
   ...
   * ```
   */
  [markdownTables]: function ({ dictionary, options }) {
    const { allTokens } = dictionary;
    const columns = normalizeMarkdownTableColumns(
      /** @type {(MarkdownTableColumn | string)[] | undefined} */ (options.columns),
    );

    const header = markdownTableRow(columns.map((column) => escapeMarkdownCell(column.header)));
    const delimiter = markdownTableRow(columns.map(() => '---'));
    const rows = allTokens.map((token) =>
      markdownTableRow(
        columns.map((column) =>
          escapeMarkdownCell(stringifyMarkdownCell(getTokenProperty(token, column.property))),
        ),
      ),
    );
