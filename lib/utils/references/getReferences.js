...
export function getReferences(value, tokens, opts = {}, references = []) {
  ...
  function findReference(_, variable) {
    ...
    if (ref === undefined && unfilteredTokens) {
      ...
      } else if (recordWarnings) {
        if (filteredReferences) {
          // collect the warning and report it once the file that is being
          // formatted is done, so it can only ever be attributed to that file
          filteredReferences.add(FILTER_WARNINGS, variable);
        } else {
          // there is no file to report this for, so warn at the record site
          // rather than parking it in a queue that any file could drain
          unfilteredWarning = `Filtered out token references were found: ${variable}`;
        }
      }
