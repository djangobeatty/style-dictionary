...
/**
 * Reference errors are hard to trace back in dictionaries with many token files,
 * so we add the reference chain that led to the error and the file it came from
 * whenever we know them.
 * @param {string} message
 * @param {{ chain?: string[], filePath?: string }} context
 * @returns {string}
 */
function withRefContext(message, { chain = [], filePath } = {}) {
  const lines = [message];
  // a chain of 2 or less is already fully described by the message itself
  if (chain.length > 2) {
    lines.push(`  Reference chain: ${chain.join(' → ')}`);
  }
  if (filePath) {
      ...
      if (typeof ref === 'string' || typeof ref === 'number') {
        ...
        if (usesReferences(to_ret, reg)) {
          ...
          if (Object.hasOwn(foundCirc, reference)) {
            ...
            const warning = withRefContext(
              `Circular definition cycle: ${circStack.join(' → ')}`,
              // the cycle itself already is the chain, so we only add the file
              { filePath },
            );
            if (throwImmediately) {
