...
/**
 * Extra information for circular reference errors when logging verbosely:
 * which files the tokens in the cycle are defined in and where the cycle was
 * detected, so users with many token files can trace where it is coming from.
 * @param {DesignTokens} tokens
 * @param {string[]} circStack
 * @param {string[]} current_context
 * @param {string} separator
 * @param {Verbosity} [verbosity]
 * @returns {string}
 */
function circularDetail(tokens, circStack, current_context, separator, verbosity) {
  if (!isVerbose(verbosity)) return '';
  let detail = '';
  const context = getName(current_context, { separator });
  const filePath = findFilePath(tokens, current_context);
  if (context && filePath) {
    detail += `\n    Detected while resolving ${context}, which is defined in file: ${filePath}`;
  }
  // the last item is a duplicate of the first, it only closes the cycle
  const cycle = [...new Set(circStack)];
  const definitions = cycle
    .map((name) => {
      const cycleFilePath = findFilePath(tokens, getPathFromName(name, separator));
      return cycleFilePath ? `        ${name}: ${cycleFilePath}` : '';
    })
    .filter(Boolean);
...
/**
 * Finds the file a token was defined in, so reference errors can point users
 * at the file they need to fix. Tokens parsed from source files are annotated
 * with the filePath they came from. Since we can be dealing with a path that
 * goes (deep) into a token's value, we walk up the object tree until we find
 * the filePath of the token the path belongs to.
 * @param {DesignTokens} tokens
 * @param {string[]} path
 * @returns {string|undefined}
 */
function findFilePath(tokens, path) {
  const currentPath = [...path];
  while (currentPath.length > 0) {
    const obj = getValueByPath(currentPath, tokens);
    if (obj && typeof obj === 'object' && obj.filePath) {
      return obj.filePath;
    }
    currentPath.pop();
  }
  ...
  value.replace(reg, (match, /** @type {string} */ variable) => {
    ...
    if (typeof ref !== 'undefined') {
      if (typeof ref === 'string' || typeof ref === 'number') {
        ...
        if (usesReferences(to_ret, reg)) {
          ...
          if (Object.hasOwn(foundCirc, reference)) {
            ...
            const detail = circularDetail(tokens, circStack, current_context, separator, verbosity);
            if (throwImmediately) {
              throw new Error(warning + detail);
            } else {
              GroupMessages.add(
                PROPERTY_REFERENCE_WARNINGS,
                'Circular definition cycle:  ' + circStack.join(', ') + detail,
              );
            }
          } else {
      ...
      if (isVerbose(verbosity)) {
        // improve traceability of the reference error by showing the chain of
        // references that led here and the file the broken reference is in
        if (stack.length > 1) {
          detail += `\n    Reference chain: ${stack.join(' -> ')}`;
        }
        const filePath = findFilePath(tokens, current_context);
        if (context && filePath) {
          detail += `\n    Token ${context} is defined in file: ${filePath}`;
        }
      }
      const warning = `Reference doesn't exist:${
        context ? ` ${context}` : ''
      } tries to reference ${variable}, which is not defined.${detail}`;
      if (throwImmediately) {
