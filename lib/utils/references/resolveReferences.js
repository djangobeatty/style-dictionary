...
export function _resolveReferences(
  ...
  value.replace(reg, (match, /** @type {string} */ variable) => {
    ...
    if (typeof ref !== 'undefined') {
      ...
      // Extra context to make it easier to trace where the broken reference
      // is coming from when there are many token files.
      const fileInfo = filePath ? `\n    in file: ${filePath}` : '';
      const chainInfo = stack.length > 2 ? `\n    reference chain: ${stack.join(' -> ')}` : '';
      const warning = `Reference doesn't exist:${
        context ? ` ${context}` : ''
      } tries to reference ${variable}, which is not defined.${fileInfo}${chainInfo}`;
      if (throwImmediately) {
        throw new Error(warning);
      } else {
