...
export function outputReferencesFilter(token, { dictionary, usesDtcg }) {
  ...
  const refs = getReferences(originalValue, dictionary.tokens, {
    ...
    // this lookup only decides whether the reference may be emitted. Every
    // reference it suppresses is cleared below, so recording a warning here
    // would only be a false positive.
    recordWarnings: false,
  });
  let allRefsFound = true;
  // Note: a plain `.every` would short-circuit on the first filtered-out ref,
  // leaving the warnings of the remaining filtered-out refs behind.
  refs.forEach((ref) => {
    ...
    if (!foundToken) {
      ...
      dictionary.filteredReferences?.remove(FILTER_WARNINGS, ref.path.join('.'));
      allRefsFound = false;
    }
  });
