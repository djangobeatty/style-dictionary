...
function convertToTokenObject(tokenArray) {
  ...
  const obj = /** @type {Tokens} */ Object.create(null);
...
export function convertTokenData(tokens, options) {
  ...
  switch (output) {
    case 'array': {
      if (input === 'object') {
        // adds "key" prop, tokens are deep cloned by flattenTokens
        ...
        return /** @type {TypeMap[T]} */ (
          /** @type {Token[]} */ (tokens).map((token) => structuredClone(token))
        );
      }
      break;
    }
    case 'map': {
      if (input === 'object') {
        ...
        return /** @type {TypeMap[T]} */ (
          convertToTokenMap(/** @type {Token[]} */ (tokens).map((token) => structuredClone(token)))
        );
      } else if (input === 'map') {
        return /** @type {TypeMap[T]} */ (
          new Map(
    ...
    case 'object': {
      if (input === 'map') {
        // we assume map is always keyed with "key" prop
        const arr = Array.from(/** @type {TokenMap} */ (tokens).values()).map((token) =>
        ...
        return /** @type {TypeMap[T]} */ (structuredClone(tokens));
      }
      break;
    }
  }
