 ...
 * @param {string} [parentFilePath] - the token file the closest ancestor token was defined in
 */
function traverseObj(slice, fullObj, opts, current_context, foundCirc, parentFilePath) {
  // tokens are tagged with the file they came from, which lets us point users
  // to the exact file a broken reference lives in
  const filePath = typeof slice.filePath === 'string' ? slice.filePath : parentFilePath;
  ...
  for (const key in slice) {
    ...
    if (typeof prop === 'object') {
      traverseObj(prop, fullObj, opts, current_context, foundCirc, filePath);
