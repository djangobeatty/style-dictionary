...
export default async function combineJSON(
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      if (!file_content) {
        // TypeScript modules are loaded through the native TypeScript support of the
        // runtime (Node.js type stripping, Deno, Bun), no transpilation step is needed
        if (['.js', '.mjs', '.ts', '.mts'].includes(extname(filePath))) {
