...
/**
 * Token file extensions that are dynamically imported as modules rather than parsed as JSON.
 */
const jsFileExtensions = ['.js', '.mjs'];
/**
 * Token file extensions that are dynamically imported as modules, but only on a runtime
 * that can execute TypeScript without a build step (Node with type stripping, Deno, Bun).
 */
...
export default async function combineJSON(
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    // tracked so a failure can be reported as a TypeScript loading problem
    // rather than as a misleading JSON parse error
    let isTypeScriptFile = false;
    try {
      ...
      if (!file_content) {
        ...
        isTypeScriptFile = tsFileExtensions.includes(ext);
        if (isTypeScriptFile || jsFileExtensions.includes(ext)) {
          ...
          file_content = (await import(/* @vite-ignore */ /* webpackIgnore: true */ importPath))
            .default;
        } else {
      ...
      if (e instanceof Error) {
        e.message = isTypeScriptFile
          ? `Failed to load TypeScript token file '${filePath}': ${e.message}. TypeScript token files require a runtime that can execute TypeScript, e.g. Node.js with type stripping (>= 22.6), Deno or Bun.`
          : 'Failed to load or parse JSON or JS Object: ' + e.message;
        throw e;
      }
    }
