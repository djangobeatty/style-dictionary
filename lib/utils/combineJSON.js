...
/**
 * File extensions that are loaded as (ES) modules rather than parsed as JSON.
 * TypeScript files can be loaded by runtimes that support it natively,
 * e.g. Deno, Bun or Node with type stripping (`process.features.typescript`).
 */
...
export default async function combineJSON(
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      if (!file_content) {
        if (moduleExtensions.includes(extname(filePath))) {
