...
export default async function combineJSON(
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      if (!file_content) {
        // TypeScript files are imported as modules as well, relying on the runtime's native
        // type stripping (Node.js >= 22.6 with --experimental-strip-types, or enabled by default
        // in Node.js >= 23.6, as well as Deno and Bun) so that no transpilation step is needed.
        if (['.js', '.mjs', '.ts', '.mts', '.cts'].includes(extname(filePath))) {
