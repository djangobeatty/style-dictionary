...
/**
 * File extensions that are loaded by dynamically importing the module and using
 * its default export, rather than being read as text and parsed.
 */
const moduleExtensions = ['.js', '.mjs', '.ts', '.mts'];
...
/**
 * Dynamically imports a JavaScript or TypeScript token module and returns its
 * default export.
 * @private
 * @param {string} filePath - Path as matched by the glob, used for error reporting
 * @param {string} resolvedPath - Path to import the module from
 * @param {string} ext - Extension of the file, including the leading dot
 * @returns {Promise<Tokens>}
 */
async function importTokenModule(filePath, resolvedPath, ext) {
  ...
  if (isNode && process?.platform === 'win32') {
    ...
    importPath = new URL(`file:///${importPath}`).href;
  }
  try {
    return (await import(/* @vite-ignore */ /* webpackIgnore: true */ importPath)).default;
  } catch (e) {
    const reason = e instanceof Error ? e.message : String(e);
    const hint =
      ext === '.ts' || ext === '.mts'
        ? ' Loading TypeScript token files requires a runtime that can import TypeScript, such as Deno, Bun, or Node.js 22.6+ with --experimental-strip-types (native from 23.6).'
        : '';
    throw new Error(`Failed to import token file "${filePath}" as a module.${hint} ${reason}`, {
      cause: e,
    });
  }
}
 ...
 * @private
 * @param {string[]} arr - Array of paths to json (or JS/TS modules that export objects) files
...
export default async function combineJSON(
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const isModule = moduleExtensions.includes(ext);
    ...
    // JS/TS token files are imported so their default export is used as token
    // data. This happens outside of the try/catch above so that an import
    // failure names the file that could not be imported instead of being
    // mislabeled as a JSON parse error.
    if (!file_content && isModule) {
      file_content = await importTokenModule(filePath, resolvedPath, ext);
    }
