...
/**
 * Extensions of files that should be loaded as ES modules rather than parsed as JSON.
 * TypeScript files rely on the runtime's native type stripping, e.g. Node.js >= 22.18
 * (or >= 22.6 with --experimental-strip-types), Bun or Deno.
 * @type {string[]}
 */
const MODULE_EXTENSIONS = ['.js', '.mjs', '.ts', '.mts', '.cts'];
...
export default async function combineJSON(
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      if (!file_content) {
        if (MODULE_EXTENSIONS.includes(extname(filePath))) {
      ...
      if (e instanceof Error) {
        if (
          TYPESCRIPT_EXTENSIONS.includes(extname(filePath)) &&
          /** @type {{ code?: string }} */ (e).code === 'ERR_UNKNOWN_FILE_EXTENSION'
        ) {
          e.message =
            `Failed to load TypeScript file "${filePath}". Your runtime does not support ` +
            `importing TypeScript files natively. Use Node.js >= 22.18 (or >= 22.6 with ` +
            `--experimental-strip-types), Bun or Deno.`;
          throw e;
        }
