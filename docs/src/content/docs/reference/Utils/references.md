...
- `recordWarnings` boolean, `true` by default. Only relevant when `warnImmediately` is `false`. When `true`, a reference that is missing from the filtered `tokens` but found in `unfilteredTokens` is recorded in a shared collector and reported once at the end of the format step. Set it to `false` for lookups that only inspect tokens without emitting a reference — reference-safe ordering, or an `outputReferences` predicate deciding whether to emit — because a warning recorded for a token that is never formatted is never cleared and is then reported as a false positive for the whole file.
