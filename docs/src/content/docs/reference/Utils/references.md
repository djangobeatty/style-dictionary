...
- `recordWarnings` boolean, `true` by default. Only relevant when `warnImmediately` is `false`. When `true`, a reference that is missing from the filtered `tokens` but found in `unfilteredTokens` is recorded in the file's collector and reported once the file has been formatted. Without a collector, the warning is printed right away instead. Set it to `false` for lookups that only inspect tokens without emitting a reference — reference-safe ordering, or an `outputReferences` predicate deciding whether to emit — because a warning recorded for a token that is never formatted is never cleared and is then reported as a false positive for the whole file.
- `filteredReferences`, the collector of the file that is currently being formatted, available as `dictionary.filteredReferences`. Style Dictionary creates one per file, so passing it is what makes a warning about a filtered-out reference be reported for the file whose filter excluded it. You only need to pass it from a custom format that resolves references itself; the built-in format helpers already do.
