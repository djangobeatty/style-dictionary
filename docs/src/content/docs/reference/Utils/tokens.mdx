...
:::note
These utilities are pure. They never modify their input, and every token they return is a deep clone (`structuredClone`), so mutating a returned token's nested properties — an object-valued `value`, `attributes`, `original`, `path` — cannot leak back into the input, and mutating the input after a call cannot change results already returned. Because each dictionary view (`tokens`, `allTokens`, `tokenMap`) is cloned independently, do not rely on object identity to carry state between them.
:::
 ...
 * `sd.tokens` is cheapest since it's already an object — it is deep cloned and returned, no flattening needed
