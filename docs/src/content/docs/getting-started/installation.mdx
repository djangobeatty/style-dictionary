...
If you open `config.json` you will see there are six platforms defined: `css`, `android`, `compose`, `ios`, `ios-swift`, and `ios-swift-separate-enums`. Each platform has a `transformGroup`, `buildPath`, and `files`. The `buildPath` and `files` of each platform should match up to the files that were built. Those files should look like these:
...
:root {
  ...
  --text-sizes-heading: 28px;
  --text-sizes-body: 16px;
}
```

**iOS**

```objectivec title="StyleDictionaryColor.h"
...
```objectivec title="StyleDictionaryColor.m"
...
1. The build system handles references to design token values in other files as well (as you can see in `tokens/text.json`, where `text.sizes.body` refers to `{dimensions.4}`).
...
:root {
  --colors-black: #111111;
  --colors-white: #ffffff;
  --borders-heavy: 4px solid #111111;
  --shadows-sm: 0px 4px 12px #111111;
}
```
